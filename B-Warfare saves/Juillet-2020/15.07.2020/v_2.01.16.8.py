...
async def on_member_join(member):
    ...
    SQL.execute('create table if not exists Accounts("Num" integer primary key autoincrement, "user_name" text, "user_id" integer NOT NULL, "Pierre" REAL, "Charbon" REAL, "Fer" REAL, "Cuivre" REAL, "Cobalt" REAL, "Gold" REAL,"Platine" REAL, "Diamant" REAL, "Palladium" REAL, "Obsidienne" REAL, "Coin" REAL)')
...
async def start(ctx):
    ...
    if result_userID is None:
        SQL.execute('insert into Accounts(user_name, user_id, Pierre, Charbon, Fer, Cuivre, Cobalt, Gold, Platine, Diamant, Palladium, Obsidienne, Coin) values(?,?,?,?,?,?,?,?,?)',(USER_NAME, USER_ID, START_BALANCE, s0, s0, s0, s0, s0, s0, s0, s0, s0, s0))
        db.commit()
...
async def mine(ctx):
    ...
    SQL.execute(f'update Accounts set Or = Or + {MOr} where user_id="{USER_ID}"')
    db.commit()
    result_userbal = SQL.fetchone()
...
async def help(ctx):
    ...
    embed = discord.Embed(
	title=f"Commande secondaire ou admin servant pour le B-Warfare",
	color = 0xfbff00
	)
...
async def i(ctx,):
    ...
    embed.add_field(name="Palladium", value=f"{result_userbal[0]} {Paladium}", inline=True)
